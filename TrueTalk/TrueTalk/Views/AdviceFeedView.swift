...
struct AdviceCardView: View {
    ...
    var body: some View {
        ...
        .frame(height: 400)
        .offset(x: localOffset.width + localDragOffset.width, y: localOffset.height + localDragOffset.height)
        .rotationEffect(.degrees(rotation + (Double(localOffset.width + localDragOffset.width) / 10)))
        ...
        .animation(.spring(response: 0.6, dampingFraction: 0.8), value: localOffset)
    ...
    private var personaHeader: some View {
        HStack(spacing: 12) {
            ...
            // Swipe indicators
            HStack(spacing: 8) {
                if localDragOffset.width < -50 {
                    Image(systemName: "xmark.circle.fill")
                        .foregroundColor(.red)
                        .font(.title2)
                        .opacity(min(abs(localDragOffset.width) / 100.0, 1.0))
                }
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                .fill(card.persona.backgroundColor.opacity(0.1) as Color)
        )
        ...
        withAnimation(.spring(response: 0.5, dampingFraction: 0.8)) {
            if abs(swipeDistance) > swipeThreshold {
                ...
                localOffset = CGSize(
                    ...
                    height: value.translation.height + Double.random(in: -100...100)
                )
